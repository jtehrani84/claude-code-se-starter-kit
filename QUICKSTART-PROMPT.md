...
1. First, ask me 5 questions:
   - What's my name, role, and OU at Salesforce?
   - What are my top 2-3 current projects?
   - What's my biggest recurring mistake or frustration with Claude?
   - What type of content do I produce most? (emails, demos, code, decks, analysis)
   - Which Salesforce products do I demo most?

2. Then build my setup:
   - Create ~/.claude/rules/ with 3 files: communication.md, security-governance.md, architecture.md
   ...
   - Create 5 skill files as custom commands: /account-prep, /deal-strategy, /email-draft, /post-meeting, /demo-prep
...
3. Wire the hooks into my settings:
   ...
   - Add PreToolUse hook pointing to guardrail.py
...
Use the templates from ~/claude-code-se-starter-kit/ as your starting point if that directory exists. Otherwise, use your best judgment based on my answers.
...
- [ ] `~/.claude/rules/` has 3 files
- [ ] Your project has a `CLAUDE.md`
- [ ] `~/.claude/hooks/scripts/` has 2 Python files
- [ ] You can run `/account-prep` or `/email-draft` successfully
