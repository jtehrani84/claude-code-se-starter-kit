...
PLIST_FILES=(
    "com.claude.gather.exa.plist"
    "com.claude.gather.hn.plist"
    "com.claude.synthesize.morning.plist"
)
...
cmd_test() {
    ...
    # Run gather scripts
    echo "=== Exa Scan ==="
    if [ -f "${SCRIPT_DIR}/gather/exa-scan.py" ]; then
        python3 "${SCRIPT_DIR}/gather/exa-scan.py" && print_status "Exa scan complete" || print_error "Exa scan failed"
    else
        print_warning "exa-scan.py not found"
    fi
...
cmd_logs() {
    ...
    LOG_DIR="${HOME}/Library/Logs/claude-crons"
    if [ -d "${LOG_DIR}" ]; then
...
case "${1:-help}" in
    ...
    logs)
        ...
        ;;
    *)
        echo "Claude Code SE Starter Kit — Cron Manager"
        echo ""
        ...
        echo ""
        echo "The pipeline:"
        echo "  4:30 AM  — exa-scan.py gathers web intelligence"
        echo "  4:45 AM  — hn-scan.py gathers developer sentiment"
