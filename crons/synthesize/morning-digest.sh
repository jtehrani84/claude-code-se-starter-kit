...
RAW_FILES=$(find "${RAW_DIR}" -name "*-${DATE}.md" -type f 2>/dev/null)
...
- Categorize: Competitive Intel | Product Updates | Account News | Industry Trends | Ecosystem
- Each item: one line with source attribution
- Flag anything requiring action with [ACTION] prefix
- Flag anything urgent (competitor launch, breaking news) with [URGENT] prefix
